...
app.get('/api/test-db', async (req, res) => {
  try {
    ...
    res.json({ success: true, timestamp: result.rows[0].now, message: 'Conexão com banco de dados OK!' });
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (userExists.rows.length) return res.status(400).json({ error: 'Email já cadastrado' });
  ...
  } catch (e) { console.error('Erro no registro:', e); res.status(500).json({ error: 'Erro ao criar usuário', details: e.message }); }
});
...
app.post('/api/auth/login', async (req, res) => {
  ...
  } catch (e) { console.error('Erro no login:', e); res.status(500).json({ error: 'Erro ao fazer login', details: e.message }); }
});

app.get('/api/auth/profile', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT id,name,email,phone,credits_balance,role,status,created_at FROM users WHERE id = $1', [req.userId]);
    if (!result.rows.length) return res.status(404).json({ error: 'Usuário não encontrado' });
    res.json({ user: result.rows[0] });
  } catch (e) { console.error('Erro ao buscar perfil:', e); res.status(500).json({ error: 'Erro ao buscar perfil', details: e.message }); }
});
  ...
  try {
    ...
    const preference = {
      items: [{
        id: package_id,
        title: pkg.name,
        description: `${pkg.credits.toLocaleString('pt-BR')} créditos para buscar leads`,
        quantity: 1,
      ...
      back_urls: {
        ...
        failure: `${process.env.FRONTEND_URL}/payment-error.html`,
      ...
      statement_descriptor: 'LEADS PARA TODOS',
      metadata: {
        user_id: user.id,
        credits: pkg.credits,
    ...
    res.status(500).json({ error: 'Erro ao criar pagamento', details: e.message });
  }
});
  ...
  try {
    ...
    if (type === 'payment') {
      ...
      if (paymentData.status === 'approved') {
        ...
        if (transResult.rows.length && transResult.rows[0].status !== 'approved') {
...
app.get('/api/payment/status/:reference', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao consultar status', details: e.message });
  }
});
...
app.get('/api/payment/transactions', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, external_reference, status, package_id, credits, amount, payment_method, created_at, approved_at FROM transactions WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50',
      [req.userId]
    );
    ...
    res.status(500).json({ error: 'Erro ao buscar transações', details: e.message });
  }
});
